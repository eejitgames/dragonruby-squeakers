...
def draw_outer_walls
  ...
  draw_wall_segment(x: 1280 - state.wall_width, y: 0, dir: :N)
  draw_wall_segment(x: 0, y: (state.segment_height + state.wall_height) * 2, dir: :N)
  draw_wall_segment(x: 1280 - state.wall_width, y: (state.segment_height + state.wall_height) * 2, dir: :N)
...
def draw_wall_segment(x:, y:, dir:)
  case dir
  when :N
    outputs.solids  << { x: x, y: y, w: state.wall_width, h: state.segment_height + (2 * state.wall_height), r: 10, g: 10, b: 200, a: 255, anchor_x: 0, anchor_y: 0, blendmode_enum: 1 }
  when :S
    outputs.solids  << { x: x, y: y - state.segment_height - state.wall_height, w: state.wall_width, h: state.segment_height + (state.wall_height * 2), r: 10, g: 10, b: 200, a: 255, anchor_x: 0, anchor_y: 0, blendmode_enum: 1 }
  when :E
    outputs.solids  << { x: x, y: y, w: state.segment_width + (2 * state.wall_width), h: state.wall_height, r: 10, g: 10, b: 200, a: 255, anchor_x: 0, anchor_y: 0, blendmode_enum: 1 }
  when :W
    outputs.solids  << { x: x - state.segment_width, y: y, w: state.segment_width + state.wall_width, h: state.wall_height, r: 10, g: 10, b: 200, a: 255, anchor_x: 0, anchor_y: 0, blendmode_enum: 1 }
  end
end
